...
function getUserId() {
  try {
    ...
    return stored || 'Miles Atwater';
  } catch {
    return 'Miles Atwater';
  }
}
...
function getAppInfo() {
  ...
  // Extract version from title like "To-Do List - WMS v0.1.0" or just use package.json version
  // For now, we'll use a default since the title doesn't include version
  const versionMatch = title.match(/v(\d+\.\d+\.\d+)/i);
  const version = versionMatch ? versionMatch[1] : '0.1.0'; // Default from package.json
  // Extract app name (everything before "v" or just use title)
  const appNameMatch = title.match(/^(.+?)\s+v\d+/i);
  const appName = appNameMatch ? appNameMatch[1].trim() : (title.split(' - ')[0] || 'To-Do List');
